...
typedef fvec_t fvec2, fvec3, fvec4; /* for better code clarity, but it should be known that these are identical in SSE and the missuse of these
                                     * size specifiers could lead to confusing bugs - hence why we still store the actual size with the vector. */
...
void fvec_add(fvec_t vout, fvec_t const v0, fvec_t const v1);
void fvec_sub(fvec_t vout, fvec_t const v0, fvec_t const v1);
void fvec_mul(fvec_t vout, fvec_t const v0, float const v1);
void fvec_cross(fvec_t vout, fvec_t const v0, fvec_t const v1);
void fvec_reverse(fvec_t vout, fvec_t const v0);
float fvec_dot(fvec_t const v0, fvec_t const v1);


static inline void fvec_scale(fvec_t *output, fvec_t const *input, float scalar)
{
    __m128 scaling_vec = _mm_load_ss(&scalar);  // avoiding use of _mm_set_ps1
